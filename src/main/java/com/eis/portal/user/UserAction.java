...
public class UserAction extends BaseAction {
	...
	public ActionForward process(ActionMapping mapping, BaseForm form, HttpServletRequest request, HttpServletResponse response, UserContext user) throws Exception {
		...
		}else if("findPm".equals(act)){
			find(request,response,Roles.PM);
			return null;
		}else if("findTm".equalsIgnoreCase(act)){
			find(request,response,Roles.TM);
			return null;
		}else if("findStuff".equalsIgnoreCase(act)){
			find(request,response,Roles.stuff);
			return null;
		}else if("getUserCheckBox".equals(act)){
			getUserCheckBox(request,response);
			return null;
		}
			return null;
	}
	
	private void getUserCheckBox(HttpServletRequest request, HttpServletResponse response)throws Exception {
		UserBO userBO = (UserBO)getBean("user_bo");
		List<UserVO> l = userBO.getUsersByRole(Roles.stuff);
		Map<String,Object> m = new HashMap<String, Object>();
		Map<String,String> selected = new HashMap<String, String>();
		for(UserVO u : l){
			m.put(u.getUser_id(), u.getUser_name());
		}
		String select = request.getParameter("selected");
		if(!CheckUtil.isEmptry(select)){
			for(String t : select.split(",")){
				selected.put(t, t);
			}
		}
		writeMessage(makeCheckbox(m, selected, "users"), response);
	}

	private ActionForward find(HttpServletRequest request,HttpServletResponse response,Roles role)throws Exception{
		String term = request.getParameter("term");
		term = URLDecoder.decode(term,"gb18030");
		term = new String(term.getBytes("iso-8859-1"));
		UserBO bo = (UserBO) getBean("user_bo");
		List l = bo.getUsersByRole(term, role);
	...
	public ActionForward add(ActionMapping mapping, BaseForm form, HttpServletRequest request, HttpServletResponse response, UserContext user) throws Exception {
		...
		//拒绝非管理员添加管理员角色
		((UserBO)bo).rejectManagerRole(selectRoles, user);
		/**检查是否正确地设置了主角色代码 
		 */
	...
	public ActionForward update(ActionMapping mapping, BaseForm form, HttpServletRequest request, HttpServletResponse response, UserContext user) throws Exception {
		...
		bo.rejectManagerRole(selectRoles, user);
		bo.rejectUpdateManager(user_id,selectRoles, user);
		/**若所选角色中，同时包括录入员和复核员，则报错，保存失败*/
		if (ipt && chk) {
	...
	public ActionForward delete(ActionMapping mapping, BaseForm form, HttpServletRequest request, HttpServletResponse response, UserContext user) throws Exception {
		...
		request.getSession().setAttribute("user_org", vo.getAmsd_store());
		((UserBO)bo).rejectDeleteManager(vo.getUser_id(), user);
	...
	public ActionForward terminate(ActionMapping mapping, BaseForm form, HttpServletRequest request, HttpServletResponse response, UserContext user) throws Exception {
		...
		bo.rejectDeleteManager(vo.getUser_id(), user);
		//调用业务对象的retrieve（）方法,获取用户状态
		vo = (UserVO) bo.retrieve(vo, user);
		request.getSession().setAttribute("user_org", vo.getAmsd_store());
