...
public class UserBO extends BaseBO {
	public static final String STAFF_LOGIC = "stuff";
	/** 
	 * 构造函数 
	 */
	public UserBO() {
	...
	public void update(BaseVO vo, UserContext user, Connection conn)
		...
		dao.delete("delete from user_depart_auth where user_id= '"+uvo.getUser_id()+"'", conn);
		String sql = "insert into user_depart_auth (user_id,depart_id) values ('"+uvo.getUser_id()+"','"+uvo.getDepart_id()+"')";
		dao.executeUpdate(sql, conn);
	...
	public void delete(BaseVO vo, UserContext user) throws Exception {
		...
		try {
			...
			dao.delete("delete from user_depart_auth where user_id= '"+((UserVO) vo).getUser_id()+"'", conn);
			//提交事务
			conn.commit();
	 ...
	 * @throws Exception
	 */
	public List getUsersByRole(String name,Roles type)throws Exception{
		UserDAO dao = (UserDAO) BeanFactory.getBean("user_dao");
		String roleId = dao.querySingle("select ROLE_ID from ep_role where LOGIC_ID='"+type+"'");
		return dao.queryList("select * from ep_user a where USER_NAME like '"+name+"%' and exists(select * from ep_user_role b where " +
				"a.USER_ID = b.USER_ID and b.ROLE_ID='"+roleId+"')");
	}
	
	public List<UserVO> getUsersByRole(Roles type)throws Exception{
		UserDAO dao = (UserDAO) BeanFactory.getBean("user_dao");
		String roleId = dao.querySingle("select ROLE_ID from ep_role where LOGIC_ID='"+type.toString()+"'");
		return dao.queryList("select * from ep_user a where  exists(select * from ep_user_role b where " +
				"a.USER_ID = b.USER_ID and b.ROLE_ID='"+roleId+"')");
	}
	
	public List<UserVO> getStaffByDeparts(String[] departs,boolean checkStatus) throws Exception{
		if(departs == null){
		...
		UserDAO dao = (UserDAO) BeanFactory.getBean("user_dao");
		StringBuffer sql = new StringBuffer("select * from ep_user where DEPART_ID in ("+result+")  and USER_ID in " +
				"(select USER_ID from ep_user_role where ROLE_ID in " +
				"(select ROLE_ID from ep_role where LOGIC_ID='"+STAFF_LOGIC+"'))");
		if(checkStatus){
			sql.append(" and STAT in ('1') ");
		}else{
			sql.append(" and STAT in ('1','4')");
		}
	...
	public UserVO getUserByUserName(String username)throws Exception{
		UserDAO dao = (UserDAO) BeanFactory.getBean("user_dao");
		StringBuffer sb = new StringBuffer("select * from ep_user where USER_NAME='"+username+"'");
		List<UserVO> l =  dao.queryList(sb.toString());
		if(l == null || l.size() == 0){
			return null;
		}
		return l.get(0);
	}
	public void rejectManagerRole(String[] roles,UserContext user)throws MessageException{
		boolean isAdmin = user.isAdmin();
		for(String s:roles){
			if(UserContext.ADMIN_ID.equals(s)){
				if(!isAdmin){
					throw new MessageException("你不是管理员，不能添加管理员角色给用户");
				}
			}
		}
	}
	public void rejectUpdateManager(String userId,String[] selectedRoles,UserContext user)throws MessageException,Exception{
		boolean isAdmin = user.isAdmin();
		if(isAdmin){
			return;
		}
		boolean isUpdateAdmin = false;
		UserRoleDAO dao = (UserRoleDAO)BeanFactory.getBean("userrole_dao");
		List<UserRoleVO> l = dao.queryList("select * from ep_user_role where USER_ID='"+userId+"'");
		for(UserRoleVO vo : l){
			if(UserContext.ADMIN_ID.equals(vo.getRole_id())){
				isUpdateAdmin = true;
			}
		}
		if(!isUpdateAdmin){
		...
		for(String s : selectedRoles){
			if(s.equals(UserContext.ADMIN_ID)){
				return;
			}
		}
		throw new MessageException("您不是管理员，无权移除管理员的角色");
	}
	public void rejectDeleteManager(String userId,UserContext user)throws MessageException,Exception{
		boolean isAdmin = user.isAdmin();
		if(isAdmin){
			return;
		}
		...
		for(UserRoleVO vo : l){
			if(UserContext.ADMIN_ID.equals(vo.getRole_id())){
				throw new MessageException("你不是管理员，不能移除管理员的角色");
			}
		}
	}

	public boolean hasRole(String logicId,UserContext user) throws Exception{
		RoleDAO roleDao = (RoleDAO)getBean("role_dao");
		StringBuffer sql = new StringBuffer();
		for(String t : user.getRole()){
			sql.append("'");
			sql.append(t);
			sql.append("',");
		}
		sql.append("' '");
		List<RoleVO> l  = roleDao.queryList("select * from ep_role where role_id in ("+sql+")");
		for(RoleVO r : l){
			if("PM".equals(r.getLogic_id())){
				return true;
			}
		}
		return false;
