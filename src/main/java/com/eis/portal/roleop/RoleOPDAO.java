	...
	public void update(String role_id, List list, UserContext user)
		...
		if (list != null) {
			...
			try {
				...
				while (ex != null) {
					SysLog.error("批量增加数据失败！",ex);
					SysLog.error("error code:" + ex.getErrorCode(),ex);
					SysLog.error("SQL State:" + ex.getSQLState(),ex);
					SysLog.error("异常信息:" + ex.getSQLState(),ex);
				...
				SysLog.error("批量增加数据失败！",t);
				SysLog.error(t.getMessage(),t);
				BaseException e = new BaseException();
				e.setErrorCode("E010012");
				throw e;
				...
				try {
					...
					SysLog.error("BaseDAO 释放数据库连接失败",ex);
					SysLog.error(ex.getMessage(),ex);
					BaseException e = new BaseException();
