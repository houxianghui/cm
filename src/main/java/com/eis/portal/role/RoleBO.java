...
public class RoleBO extends BaseBO {
	...
	* user:用户对象 
	 ...
	 * @throws Exception
	 */
	public List<String> getUsersByRole(String logicCode)throws Exception{
		DBUtil db = new DBUtil();
		
		//模糊匹配条件
		StringBuffer sql =new StringBuffer("select * from ep_user_role a where exists (select * from ep_role b where b.LOGIC_ID='"+logicCode+"' and a.ROLE_ID = b.ROLE_ID)");
		List<String> result = new ArrayList<String>();
		try{
			ResultSet rs = db.sqlQuery(sql.toString());
			while(rs.next()){
