...
public final class StringUtil {
	...
	public static String transStr(String str,String originCharset,String destCharset) throws Exception {
		try {
			...
			SysLog.error("字符串转码失败：" + ex.getMessage(),ex);
		}
		return str;
	}
	...
	public static String[] getTypes(){
		ProjectType[]  pt = ProjectType.values();
		DailyType[] dt = DailyType.values();
		String[] result = new String[pt.length+dt.length];
		int j = 0;
		for(int i = 0;i<pt.length;i++,j++){
			result[j] = pt[i].toString();
		}
		for(int i = 0;i<dt.length;i++,j++){
			result[j] = dt[i].toString();
		}
		return result;
	}
	public static String[] getTypesDesc(){
		ProjectType[]  pt = ProjectType.values();
		DailyType[] dt = DailyType.values();
		String[] result = new String[pt.length+dt.length];
		int j = 0;
		for(int i = 0;i<pt.length;i++,j++){
			result[j] = pt[i].getDesc();
		}
		for(int i = 0;i<dt.length;i++,j++){
			result[j] = dt[i].getDesc();
		}
		return result;
	}
