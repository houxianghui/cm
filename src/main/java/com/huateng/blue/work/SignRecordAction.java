...
public class SignRecordAction extends IbatisAction {
	...
	public ActionForward process(ActionMapping mapping, BaseForm form,
		...
		if("info".equals(act)){
			return info(mapping,request,user,form);
		}
		if("new".equals(act)){
	...
	private ActionForward info(ActionMapping mapping,HttpServletRequest request,UserContext user,BaseForm form)throws Exception{
		((SignRecordBO)bo).setMySignListInfo(user, request,form);
		return mapping.findForward("list");
	}
