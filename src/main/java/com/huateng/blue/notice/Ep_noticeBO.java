...
public class Ep_noticeBO extends BaseBO { 
	...
	public String getNotice(boolean flag)throws Exception{
		...
		tr.addTD(new HtmlTD("序号"));
		tr.addTD(new HtmlTD("内容"));
		tr.addTD(new HtmlTD("日期"));
		tr.addTD(new HtmlTD("下载"));
		table.addTr(tr);
		int i = 1;
		while(it.hasNext()){
			tr = new HtmlTR();
			HtmlTD td = new HtmlTD(new HtmlLabel(""+i++));
			tr.addTD(td);
			vo = (Ep_noticeVO)it.next();
			td = new HtmlTD(new HtmlLabel(vo.getNotice_comment()));
			tr.addTD(td);
			tr.addTD(new HtmlTD(new HtmlLabel(vo.getOper_date())));
			
			if(!vo.getFilename().equals("")){
				tr.addTD(new HtmlTD(new HtmlLabel("<a href='Ep_notice.do?act=download&fileName="+URLEncoder.encode(URLEncoder.encode(vo.getFilename(),"utf-8"),"utf-8")+"'>"+vo.getFilename()+"</a>")));
			}else{
				tr.addTD(new HtmlTD(new HtmlLabel("")));
			}
			table.addTr(tr);
			if(i == 6){
				map.put("five",table.toString()+"<a href='Ep_notice.do?act=pop&flag=1'>更多>>..</a><br>");		
			}
		}
		map.put("more",table.toString());
		while(it.hasNext()){
			vo = (Ep_noticeVO)it.next();
		}
		if(flag == true || i <= 6){
