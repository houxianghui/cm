...
public class ParamChgAction extends IbatisBaseAction {
	@Autowired
	private ParamChgBO bo;
	@Override
	...
	private ActionForward delete(ActionMapping mapping, BaseForm form, HttpServletRequest request, UserContext user) throws Exception{
		ParamChg v = bo.queryForObject(request.getParameter("id"));
		bo.delete(v.getId());
	...
	private ActionForward list(ActionMapping mapping, BaseForm form, HttpServletRequest request, UserContext user) throws Exception{
		...
		((ParamChgForm)form).setVersionId(request.getParameter("versionId"));
		return mapping.findForward("list");
	}
	private String getBackUrl(ParamChg v){
		return "VersionHis.do?act=tabs&versionId="+v.getVersionId()+"&type="+DetailType.param;
	}
}
