...
public class DatabaseChgAction extends IbatisBaseAction {
	@Autowired
	private DatabaseChgBO bo ;
	@Override
	...
	private ActionForward delete(ActionMapping mapping, BaseForm form, HttpServletRequest request, UserContext user) throws Exception{
		DatabaseChg v = bo.queryForObject(request.getParameter("id"));
		bo.delete(v.getId());
	...
	private ActionForward list(ActionMapping mapping, BaseForm form, HttpServletRequest request, UserContext user) throws Exception{
		...
		((DatabaseChgForm)form).setVersionId(request.getParameter("versionId"));
		return mapping.findForward("list");
	}
	private String getBackUrl(DatabaseChg v){
		return "VersionHis.do?act=tabs&versionId="+v.getVersionId()+"&type="+DetailType.database;
	}
