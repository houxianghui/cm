...
function setPKey(id,name) { 
	opener.document.forms[0].<%=projectDistributeForm.getId_field()%>.value=id;
	opener.document.forms[0].<%=projectDistributeForm.getName_field()%>.value=name;
	window.close();
} 
$(function(){
	...
	<table width="98%" class="dtPanel_Line1" border="0" cellspacing="1"
		...
		<tr align="left" class="dtPanel_Main" onclick="_clickTr( this )">
			...
			<td><%=vo.getReminingDays() %>天</td>
					...
					<td><%=vo.getMemo()%></td>
					<td><%=msg%></td>
					<td><label><input type="radio" name="param"	onClick="setPKey('<%=vo.getProjectId()%>','<%=vo.getProject().getProjectName()%>')">
					</label></td>
		</tr>
