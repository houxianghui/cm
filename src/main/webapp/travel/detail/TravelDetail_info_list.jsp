...
<jsp:useBean id="travelDetailForm" scope="request" class="com.blue.travel.detail.TravelDetailForm"/>
...
function doEdit(){ 
	//修改 
	//检查是否有选中的纪录 
	if(document.forms[0].tdetailId.value == null ||document.forms[0].tdetailId.value == "") { 
		alert('请先选择纪录'); 
		return; 
	} 
	//提交表单 
...
function setPKey(id1,id2) { 
	document.forms[0].tdetailId.value=id1; 
		...
		<table width="98%" class="dtPanel_Line1" border="0" cellspacing="1" align="center" cellpadding="0"> 
			...
			%> 
					<tr align="center" class="dtPanel_Main" onclick="_clickTr( this )"> 					
						<td><%=vo.getTravelFrom()%></td>
						<td><%=vo.getTravelTo()%></td>
						<td><%=vo.getTransportMode()%></td> 
						<td><%=vo.getInvoiceno()%></td> 
						<td><%=vo.getFee()%></td> 
						<td><label><input type="radio" name="param"  onClick="setPKey('<%=vo.getTdetailId()%>','<%=vo.getExpensesId()%>')"> </label></td> 
					</tr> 
