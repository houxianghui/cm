...
<%@ page import="com.blue.travel.TravelInfo" %> 
...
function doAdd(){ 
	//增加 
	window.location="TravelInfo.do?act=c"; 
} 
...
function setPKey(travel_id_var) { 
	document.forms[0].expensesId.value=travel_id_var; 
} 
			...
			<% 
				if(pageResult != null)  { 
					...
					if(list != null ) { 
						...
						while (iter.hasNext()) { 
							TravelInfo vo = (TravelInfo) iter.next(); 
			%> 
			...
			<br> 
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
					<tr> 
						...
						<input type="button" class="Button" name="add" id="travel_info_c" value="增加" onClick="doAdd()"/> &nbsp; 
						<input type="button" class="Button"  name="edit" id="travel_info_u" value="修改" onClick="doEdit()"/> &nbsp;  
						<input type="button" class="Button"  name="delete" id="travel_info_d" value="删除" onClick="doDelete()"/>  &nbsp;  
						<input type="button" class="Button"  name="print" id="travel_info_p" value="打印页面" onClick="doPrint()"/> &nbsp;
