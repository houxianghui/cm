...
function doDepart(){
	...
	if(document.forms[0].status.value=="作废"){
		alert('不能对已废除角色进行用户角色配置管理！');
		return;
	}
	
	//到角色操作权限管理页面		
	window.location="UserDepartAuth.do?act=init&role_id="+document.forms[0].role_id.value;
}
function doPerm(){
	//权限管理
...
<table  class=heightspace_top3 ><tr><td></td></tr></table>
		...
		<table width="98%" align="center" border="0" cellspacing="0" cellpadding="0">					
				<tr >
					...
					<input type=button class=Button name="depart" id="role_depart"  value="用户部门权限" onClick="doDepart()"/> &nbsp;
